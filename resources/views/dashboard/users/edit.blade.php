...
@section('content')
    <form action="{{ route('dashboard.users.update',$user->id) }}" method="post" enctype="multipart/form-data"
        ...
        <div class="form-group">
            ...
            <img id="blah" style="width: 100px" src="{{ $user->image_path ?? asset('uploads/users_images/default.jpg') }}" alt="your image" />
        </div>
        ...
        <div class="card-body">
            ...
            <div class="tab-content" id="custom-content-below-tabContent">
                @foreach ($models as $index => $model)
                    <div class="tab-pane fade {{ $index == 0 ? 'show active' : '' }}" id="custom-content-below-{{ $model }}" role="tabpanel"
                        aria-labelledby="custom-content-below-home-tab">
                        @foreach ($maps as $map)
                            <label><input type="checkbox" {{ ($user->hasPermission($model . '_' . $map) ? 'checked' : '' ) }} name="permissions[]" value="{{ $model . '_' . $map }}">
                                @lang('site.' . $map)</label>
                        @endforeach
                    </div>
                    @endforeach
